    ...
    public static void sort(final List<Stock> list) {
        Collections.sort(list, new Comparator<Stock>() {
            public int compare(Stock stock1, Stock stock2) {
                ...
                int comparedNames = ((String) stock1.get("name")).compareTo((String) stock2.get("name"));

                if (comparedNames != 0) return comparedNames;

                else
                {
                    int compareDates = ((Date) stock1.get("date")).compareTo((Date) stock2.get("date"));
                    ...
                    if (compareDates != 0) return compareDates;
                        ...
                        if (stock1.containsKey("change")){
                            income1st = ((Double) stock1.get("change"));
                        }
                        else
                            income1st = (Double) stock1.get("last") - (Double) stock1.get("open");

                        if (stock2.containsKey("change")){
                            income2nd = ((Double) stock2.get("change"));
                        }
                        else
                            income2nd =  (Double) stock2.get("last") - (Double) stock2.get("open");

                            int compareChange = (income2nd.compareTo(income1st));
                            if (compareChange != 0)
