...
public class Solution {
    ...
    public void recursion(final String expression, int countOperation) {
        ...
        if (p3 != -1) {
            ...
            switch (s[p3]) {
                case '^':
                    customPrint(decimalFormat, Math.pow(Double.parseDouble(part1[0]), Double.parseDouble(part2[0])), countOperation, currentCountOperation);
                    return;
                case '*':
                    customPrint(decimalFormat, Double.parseDouble(part1[0]) * Double.parseDouble(part2[0]), countOperation, currentCountOperation);
                    return;
                case '/':
                    customPrint(decimalFormat, Double.parseDouble(part1[0]) / Double.parseDouble(part2[0]), countOperation, currentCountOperation);
                    return;
        ...
        if (s.length > 5 && Character.isAlphabetic(s[0]) && s[3] == '(' && s[s.length - 1] == ')') {
            ...
            if ("sin".equals(funcName)) {
                customPrint(decimalFormat, Math.sin(Math.toRadians(Double.parseDouble(part[0]))), countOperation, currentCountOperation);
                return;
            }
            if ("cos".equals(funcName)) {
                customPrint(decimalFormat, Math.cos(Math.toRadians(Double.parseDouble(part[0]))), countOperation, currentCountOperation);
                return;
            }
            if ("tan".equals(funcName)) {
                customPrint(decimalFormat, Math.tan(Math.toRadians(Double.parseDouble(part[0]))), countOperation, currentCountOperation);
                return;
            }
        }
        double n = 0d;
        ...
        customPrint(decimalFormat, n, countOperation, countOperation);
    }
