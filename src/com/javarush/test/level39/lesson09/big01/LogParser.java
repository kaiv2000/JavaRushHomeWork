...
public class LogParser implements IPQuery, UserQuery, DateQuery, EventQuery, QLQuery {
    ...
    public LogParser(Path logDir) {
        this.logDir = logDir;
        allLinesList = getLinesList();
    }

    public String[] getFileList() {
        File files = logDir.toFile();
    ...
    private List<String> getLinesList() {
        ...
        for (String oneLogFile : fileList) {
            String oneFile = logDir + File.separator + oneLogFile;
            try (BufferedReader reader = new BufferedReader(new FileReader(oneFile))) {
                String oneLine = "";
                while ((oneLine = reader.readLine()) != null) {
                    allLinesList.add(oneLine);
                }
            } catch (IOException e) {
                e.printStackTrace();
            }
    ...
    public Set<String> getUniqueIPs(Date after, Date before) {
        ...
        for (String oneLine : allLinesList) {
    ...
    public Set<String> getIPsForUser(String user, Date after, Date before) {
        ...
        for (String oneLine : allLinesList) {
            ...
            if (user.equals(parts[1])) {
                addStringEntity(after, before, ipForUser, parts, 0);
            }
        }
        return ipForUser;
    }
    ...
    public Set<String> getIPsForEvent(Event event, Date after, Date before) {
        ...
        for (String oneLine : allLinesList) {
            String[] parts = oneLine.split("\\t");
            if (event.toString().equals(parts[3].split(" ")[0])) {
                addStringEntity(after, before, ipForEvent, parts, 0);
            }
        }
    ...
    public Set<String> getIPsForStatus(Status status, Date after, Date before) {
        ...
        for (String oneLine : allLinesList) {
            ...
            if (status.toString().equals(parts[4])) {
                addStringEntity(after, before, ipForStatus, parts, 0);
            }
        }
        return ipForStatus;
    }
